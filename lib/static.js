...
module.exports = async () => {
  ...
  // copy plugin in current folder
  const currentDir = process.cwd();//getPath();
  console.log(`current dir: ${currentDir}`);
  const pluginFolder = path.join(currentDir, 'plugin');
  console.log('pluginFolder', pluginFolder);
  fs.readdir(pluginFolder, (err, files) => {
    const targetPluginFolder = path.join(staticDir, 'plugin');
    fs.ensureDirSync(targetPluginFolder);
    files.forEach(subfolder => {
      const targetPath = path.join(targetPluginFolder, subfolder);
      cp(path.join(pluginFolder, subfolder), targetPath);
    });
  });
